...
import type { ChannelUpdatedEvent, UserProfileRedis } from 'mezon-js';
...
export type DirectMessageHeader = Omit<ApiChannelMessageHeader, 'id' | 'sender_id'> & {
	id: string;
	sender_id?: string;
};

export interface DirectEntity extends Omit<IChannel, 'last_sent_message' | 'last_seen_message'> {
	id: string;
	showPinBadge?: boolean;
	last_sent_message?: DirectMessageHeader;
	last_seen_message?: DirectMessageHeader;
}
...
export const mapDmGroupToEntity = (channelRes: ApiChannelDescription, existingEntity?: DirectEntity): DirectEntity => {
	const mapped: DirectEntity = {
		...channelRes,
		...
		last_sent_message: channelRes.last_sent_message
			? {
					...channelRes.last_sent_message,
					id: channelRes.last_sent_message.id ? String(channelRes.last_sent_message.id) : '',
					sender_id: channelRes.last_sent_message.sender_id ? String(channelRes.last_sent_message.sender_id) : undefined
				}
			: undefined,
		last_seen_message: channelRes.last_seen_message
			? {
					...channelRes.last_seen_message,
					id: channelRes.last_seen_message.id ? String(channelRes.last_seen_message.id) : '',
					sender_id: channelRes.last_seen_message.sender_id ? String(channelRes.last_seen_message.sender_id) : undefined
				}
			: undefined
	};
...
export const fetchDirectDetail = createAsyncThunk('direct/fetchDirectDetail', async ({ directId }: { directId: string }, thunkAPI) => {
	try {
		...
		const response = await withRetry(() => mezon.client.listChannelDetail(mezon.session, BigInt(directId)), {
			maxRetries: 3,
			initialDelay: 1000,
			scope: 'dm-detail'
		});
			...
			if (response) {
					...
					directActions.upsertOne({
						...
						last_seen_message: response.last_seen_message
							? {
									...
									id: response.last_seen_message.id ? String(response.last_seen_message.id) : '',
									sender_id: response.last_seen_message.sender_id ? String(response.last_seen_message.sender_id) : undefined
								}
							: undefined
					})
				);
				...
				await thunkAPI.dispatch(
					channelsActions.joinChat({
						clanId: '0',
						channelId: String(response.channel_id) as string,
...
export const closeDirectMessage = createAsyncThunk('direct/closeDirectMessage', async (body: ApiDeleteChannelDescRequest, thunkAPI) => {
	try {
		...
		if (response) {
			thunkAPI.dispatch(directActions.setDmActiveStatus({ dmId: String(body.channel_id) as string, isActive: false }));
			return response;
		} else {
	...
	async ({ channelId, clanId }: { channelId: string; clanId: string }, thunkAPI) => {
		try {
			...
			if (dmChannel?.active !== ActiveDm.OPEN_DM && clanId === '0') {
				await mezon.client.openDirectMess(mezon.session, { channel_id: BigInt(channelId) });
			}
		} catch (error) {
		...
		try {
			...
			if (Date.now() - response.time < 100) {
				const listStatusUnreadDM = response.channeldesc.map((channel) => {
					...
					return { dmId: String(channel.channel_id) ?? '', isUnread: status };
				});
			...
			const channels = sorted.map((channelRes) => {
				const existingEntity = existingEntities.find((entity) => entity.id === String(channelRes.channel_id));
...
export const updateDmGroup = createAsyncThunk(
	...
	async (body: { channel_id: string; channel_label?: string; channel_avatar?: string }, thunkAPI) => {
		try {
			...
			const response = await mezon.client.updateChannelDesc(mezon.session, BigInt(body.channel_id), updatePayload);
...
const mapMessageToConversation = (message: IMessage): DirectEntity => {
	return {
...
export const addGroupUserWS = createAsyncThunk('direct/addGroupUserWS', async (payload: AddGroupUserWSPayload, thunkAPI) => {
	try {
		...
		const state = thunkAPI.getState() as RootState;
		const existingEntity = selectAllDirectMessages(state).find((entity) => entity.id === String(channel_desc.channel_id));

		const directEntity: DirectEntity = {
			...channel_desc,
			id: String(channel_desc.channel_id) || '',
			user_ids: userIds,
			usernames,
			display_names: label,
			...
			last_sent_message: channel_desc.last_sent_message
				? {
						...channel_desc.last_sent_message,
						id: channel_desc.last_sent_message.id ? String(channel_desc.last_sent_message.id) : '',
						sender_id: channel_desc.last_sent_message.sender_id ? String(channel_desc.last_sent_message.sender_id) : undefined
					}
				: undefined,
			last_seen_message: channel_desc.last_seen_message
				? {
						...channel_desc.last_seen_message,
						id: channel_desc.last_seen_message.id ? String(channel_desc.last_seen_message.id) : '',
						sender_id: channel_desc.last_seen_message.sender_id ? String(channel_desc.last_seen_message.sender_id) : undefined
					}
				: undefined
		};
		thunkAPI.dispatch(
...
export const directSlice = createSlice({
	...
	reducers: {
		...
		updateE2EE: (state, action: PayloadAction<Partial<ChannelUpdatedEvent & { currentUserId: string }>>) => {
			...
			const existingDirect = state.entities[String(channel_id)];
		...
		addMemberDmGroup: (state, action: PayloadAction<DirectEntity>) => {
			const dmGroup = state.entities?.[String(action.payload.channel_id)];
			if (dmGroup) {
		...
		updateMoreData: (state, action: PayloadAction<DirectEntity>) => {
			...
			const currentData = state.entities[String(channelId)];
			...
			if (String(data?.clan_id) === '0' && currentData?.active !== ActiveDm.OPEN_DM) {
		...
		setDirectLastSeenTimestamp: (state, action: PayloadAction<{ channelId: string; timestamp: number; messageId?: string }>) => {
			...
			const entity = state.entities[channelId];
			const lastSeenMessage: DirectMessageHeader = {
				...(entity?.last_seen_message || {}),
				timestamp_seconds: Math.floor(timestamp),
				id: messageId || entity?.last_seen_message?.id || ''
			};
		...
		setDirectMetaEntities: (state, action: PayloadAction<DirectEntity[]>) => {
			const channels = action.payload;
			if (channels) {
				for (const ch of channels) {
					const entity = state.entities[String(ch.channel_id) || ''];
					if (entity) {
						const changes: Partial<DirectEntity> = {};
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(fetchDirectMessage.fulfilled, (state: DirectState, action: PayloadAction<DirectEntity[]>) => {
