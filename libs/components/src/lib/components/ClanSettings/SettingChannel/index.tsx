...
import { createImgproxyUrl, generateE2eId, getDateLocale, normalizeString } from '@mezon/utils';
...
import PanelChannelTable from '../../PanelChannel/PanelChannelTable';
import type { ChannelSortDirection, ChannelSortField } from './SortableChannelHeader';
import { SortableChannelHeader } from './SortableChannelHeader';

const compareChannels = (a: ApiChannelSettingItem, b: ApiChannelSettingItem, field: ChannelSortField) => {
	switch (field) {
		case 'name':
			return normalizeString(a.channel_label ?? '').localeCompare(normalizeString(b.channel_label ?? ''));

		case 'members':
			return (a.user_ids?.length ?? 0) - (b.user_ids?.length ?? 0);

		case 'messagesCount':
			return Number(a.message_count ?? 0) - Number(b.message_count ?? 0);

		case 'lastSent':
			return Number(a.last_sent_message?.timestamp_seconds ?? 0) - Number(b.last_sent_message?.timestamp_seconds ?? 0);
...
const ListChannelSetting = ({ listChannel, clanId, countChannel, searchFilter }: ListChannelSettingProp) => {
	...
	const [sortField, setSortField] = useState<ChannelSortField | null>(null);
	const [sortDirection, setSortDirection] = useState<ChannelSortDirection>('asc');
	...
	const sortedChannels = useMemo(() => {
		if (!listChannel || !sortField) {
			return listChannel ?? [];
		}

		return [...listChannel].sort((a, b) => {
			const result = compareChannels(a, b, sortField);

			return sortDirection === 'asc' ? result : -result;
		});
	}, [listChannel, sortField, sortDirection]);
	...
	const handleChangePageSize = useCallback(
		...
		[clanId, dispatch, listChannel.length, pageSize]
	);
	...
	const channelListCut = useMemo(() => {
		...
		if (start >= sortedChannels.length) {
			const lastPage = Math.ceil(sortedChannels.length / pageSize);
			start = (lastPage - 1) * pageSize;
			end = start + pageSize;
		}

		return sortedChannels.slice(start, end);
	...
	return (
		<div className="h-full w-full flex flex-col gap-1 flex-1">
			<div className="flex flex-row justify-between items-center px-4 h-12 shadow border-b-theme-primary">
				<SortableChannelHeader
					field="name"
					label={t('table.columnHeaders.name')}
					className="flex-1"
					sortField={sortField}
				...
				<SortableChannelHeader
					field="members"
					label={t('table.columnHeaders.members')}
				...
				<SortableChannelHeader
					field="messagesCount"
					label={t('table.columnHeaders.messagesCount')}
					className="flex-1 justify-center"
					sortField={sortField}
					sortDirection={sortDirection}
					setSort={setSort}
				...
				<SortableChannelHeader
					field="lastSent"
					label={t('table.columnHeaders.lastSent')}
					className="flex-1 justify-center"
					sortField={sortField}
					sortDirection={sortDirection}
					setSort={setSort}
				...
				<SortableChannelHeader
					field="creator"
					label={t('table.columnHeaders.creator')}
					className="flex-1 justify-center"
					sortField={sortField}
					sortDirection={sortDirection}
					setSort={setSort}
			...
			<div className="relative" onClick={handleFetchThreads}>
				...
				{!isVoiceChannel && !searchFilter && (
					<div
						...
						className={`absolute top-4 right-2 cursor-pointer transition duration-100 ease-in-out flex items-center justify-center h-6 w-6 ${
							showThreadsList ? '' : '-rotate-90'
						}`}
					>
						{isLoadingThreads ? (
			...
			{showThreadsList && !searchFilter && (
				<div className="flex flex-col pl-8">
					{threadsList?.length > 0 ? (
						...
						<div className="w-full py-4 relative before:content-[' '] before:w-full before:h-[0.08px] before:absolute before:top-0 before:left-0 group text-textPrimaryLight dark:text-textPrimary">
...
const ItemInfor = ({
	...
	const [isShowPanelChannel, setIsShowPanelChannel] = useState(false);
		...
		return (
			...
			>
				...
				>
					...
					{userIds.map((member) => (
						<div className="flex gap-3" key={member}>
							<AvatarUserShort id={member} showName />
						</div>
					))}
				</div>
			</div>
		...
		return (
			...
			>
				<ChannelRow
					...
					isStream={isStream}
					isApp={isApp}
					isAgeRestricted={isAgeRestrictedChannel}
					...
					creatorDisplayName=""
					creatorAvatar=""
					handleShowAllMemberList={handleShowAllMemberList}
				/>
			</div>
		);
	}
		...
		<>
			...
			>
				<ChannelRow
					...
					date={date}
					numberFormatter={numberFormatter}
					creatorDisplayName={creatorChannel?.clan_nick || creatorChannel?.user?.display_name || creatorChannel?.user?.username || ''}
					creatorAvatar={creatorChannel?.clan_avatar || creatorChannel?.user?.avatar_url || ''}
					handleShowAllMemberList={handleShowAllMemberList}
				/>
			</div>
			...
			{isShowPanelChannel && (
				<PanelChannelTable
					coords={coords}
					channel={channelEntity as IChannel}
...
const ChannelRow = ({
	...
	return (
		...
		>
				...
				{!isVoice &&
					...
					(isThread ? (
						privateChannel ? (
							...
							<Icons.ThreadIcon defaultFill1="var(--bg-icon-theme)" defaultFill4="var(--bg-theme-secounnd)" />
						)
					) : isAgeRestricted ? (
			...
			>
				{numberFormatter.format(Number(messageCount || 0))}
			</div>
			...
			<div className="overflow-hidden flex w-12 items-center justify-center">
				<AvatarImage
					...
					src={creatorAvatar}
					srcImgProxy={createImgproxyUrl(creatorAvatar, {
						width: 32,
						height: 32,
						resizeType: 'fit'
					})}
...
export const AvatarUserShort = ({ id, showName = false }: { id: string; showName?: boolean }) => {
	...
	return (
		<div className="flex items-center gap-3" data-e2e={generateE2eId('clan_page.channel_list.item.user_list_collapsed.item')}>
			...
			{showName ? <div>{displayName}</div> : null}
		</div>
	);
};
