...
/**
 * Fallback for browsers without MediaStreamTrackProcessor (Firefox, Safari).
 *
 * Same renderer as the worker path, but driven by requestAnimationFrame on the
 * main thread — which the browser throttles once the tab is hidden, so the caller
 * is told via `onVisibilityDegraded` and can warn the user.
 */
export class CanvasCompositor implements RecordingCompositor {
	...
	constructor(width: number, height: number, fps: number) {
		...
		// Safari stops decoding video elements that are not in the document.
