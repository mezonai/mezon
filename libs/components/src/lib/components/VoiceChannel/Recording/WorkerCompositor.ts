...
interface SourceEntry {
	/** Clone, so the recorder never interferes with the track the UI renders. */
...
/**
 * Highest-performance path (Chromium): every frame is decoded, composited and
 * handed back as a track without the main thread touching a pixel.
 *
 *   remote track ──▶ MediaStreamTrackProcessor.readable ─(transfer)─▶ worker
 *   worker ──▶ OffscreenCanvas ──▶ VideoFrame ──▶ MediaStreamTrackGenerator.writable
 *   generator (a MediaStreamTrack on this thread) ──▶ MediaRecorder
 */
export class WorkerCompositor implements RecordingCompositor {
	...
	setVideoSources(next: Map<string, MediaStreamTrack>): void {
		...
		for (const [key, track] of next) {
			...
			try {
				...
				// A small buffer keeps latency low; stale frames are worthless here.
	...
	async stop(): Promise<void> {
		...
		await new Promise<void>((resolve) => {
			...
			// The worker still has to flush its writer, but never let that block a leave.
