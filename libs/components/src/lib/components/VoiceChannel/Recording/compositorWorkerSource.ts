...
/**
 * Body of the compositor worker.
 *
 * Like `renderRecordingFrame`, this is serialised with `toString()` and evaluated
 * in a blob worker (the pattern already used by `launchMediaWorkers`), which keeps
 * it bundler-agnostic — Vite and Webpack both ship it untouched. It therefore must
 * not reference anything outside its own body; the renderer arrives via `helpers`.
 *
 * Frames are pulled off the media pipeline (`MediaStreamTrackProcessor`) rather than
 * requestAnimationFrame, so compositing keeps running at full rate while the tab is
 * in the background, and the main thread does no per-frame work at all.
 */
export function compositorWorkerMain(helpers: { render: (...args: any[]) => void }): void {
	...
	function renderOnce(): void {
		...
		// Back-pressure: the generator has not drained yet, so drop rather than queue.
		...
		// The writable takes ownership of the frame; it must not be closed here.
	...
	function tick(): void {
		...
		// Drift-corrected pacing keeps frame timestamps locked to the wall clock.
...
/**
 * Serialised worker bundle. Both functions are self-contained, so the minified
 * output of either one is still valid standalone source.
 */
