...
const VAR_FILLS: Pick<ComponentProps<typeof Icons.AppChannelIcon>, 'defaultFill1' | 'defaultFill2' | 'defaultFill3'> = {
...
type AppChannelListIconProps = Omit<ComponentProps<typeof Icons.AppChannelIcon>, 'defaultFill1' | 'defaultFill2' | 'defaultFill3'> & {
...
export const AppChannelListIcon = memo(function AppChannelListIcon({ isEmphasized, className = 'w-5 h-5', ...props }: AppChannelListIconProps) {
	return <Icons.AppChannelIcon className={`${className} ${appChannelListIconFillClassName(isEmphasized)}`.trim()} {...VAR_FILLS} {...props} />;
});
