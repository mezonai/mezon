...
import { EStateFriend, FriendsEntity, selectAllFriends, useAppSelector } from '@mezon/store-mobile';
...
export const BlockedUsers = ({ navigation }: SettingScreenProps<BlockedUsersScreen>) => {
	...
	const blockedUsers = allFriends?.filter((friend) => friend?.state === EStateFriend?.BLOCK);
	const { unBlockFriend } = useFriends();

	const handleUnblockFriend = async (user: FriendsEntity) => {
		try {
			const targetId = user?.id;
			const targetUsername = user?.user?.username;
