...
const ClansList = memo(() => {
	...
	const {
		...
		handleMouseEnter,
		handleDragOverItem
	} = useClanDragAndDrop(items, setItems, clanGroups);
	...
	const clanMap = clans.reduce((acc, c) => {
		acc[c.id] = c;
		return acc;
			...
			{items.map((id, index) => {
				...
				if (group) {
					return (
						<Fragment key={id}>
							...
							<div
								className="relative"
								onMouseDown={(e) => handleMouseDown(e, id, 'group')}
				...
				return (
					<Fragment key={id}>
						...
						<div
							className="relative"
							onMouseDown={(e) => handleMouseDown(e, id, 'clan')}
							onMouseEnter={() => handleMouseEnter(id)}
						>
							{isBeingDragged ? (
								<div className="w-[40px] h-[40px] rounded-lg border-2 border-dashed border-gray-400 bg-sky-200 opacity-50" />
